...
def missing_elements(my_list: list) -> list:
    if not my_list:
        return []
    my_set = set(my_list)
    full_range = range(min(my_list), max(my_list) + 1)
    return [x for x in full_range if x not in my_set]
    pass
"""
...
def char_frequency(my_string: str) -> dict:
    my_dict = {key: 0 for key in set(my_string)}
...
def word_frequency(my_string: str) -> dict:
    words = my_string.split()
    my_dict = {key: 0 for key in set(words)}
    for word in words:
        my_dict[word] += 1
    return my_dict
    pass
...
def count_in_range(my_list: list, start: int, end: int) -> int:
    filtered_elements = [x for x in set(my_list) if start <= x <= end]
...
def list_intersection(list1: list, list2: list) -> list:
    set2 = set(list2)
    return list(set(element for element in list1 if element in set2))
    pass
...
def most_frequent(my_list: list) -> int:
    ...
    for item in my_set:
        ...
        for i in my_list:
            if i == item:
                counter += 1
        if counter > great_counter:
            great_counter = counter
            freq_num = item
    return freq_num
    pass
"""
...
def least_frequent(my_list: list) -> int:
    ...
    for item in my_list:
        if item in my_dict:
            ...
            my_dict[item] = 1
    least_frequent_item = my_list[0]
    min_count = my_dict[least_frequent_item]
